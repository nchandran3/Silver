...
public class BuyFoodTest {
	...
	public void testBuyFoodFF(){
		...
		assertFalse("Owner want to buy too many ", store.buyFood(np, store.getFood() + 1));
	...
	public void testBuyFoodTF(){
		...
		assertTrue("Owner doesnt have enough money", np.getGold() > store.getFood()*store.getFoodPrice());
	...
	public void testBuyFoodTT(){
		...
		assertTrue("Make Sure there is a price for food", price > 0);
		assertTrue("Owner want to buy all", store.buyFood(np, store.getFood()/2));
		assertTrue("Owner doesnt have enough money", np.getGold() > store.getFood()*store.getFoodPrice());
	...
	public void testBuyFoodFT(){
		...
		assertTrue("Make Sure there is a price for food", price > 0);		
		assertFalse("Owner want to buy too many ", store.buyFood(np, store.getFood() + 1));
		assertTrue("Player has ebough Gold", np.getGold()>store.getFood()*0);
