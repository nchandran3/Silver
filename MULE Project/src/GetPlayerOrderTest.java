...
public class GetPlayerOrderTest {
	/*
	 *  Player score is calculated by: food + dragonFire + ore + gold*propertiesOwned
	 *  The different Races start out with different amounts of gold:
	 *  TARGARYEN = 600
	 *  GREYJOY = 1000
	 *  BARATHEON = 1000
	 *  STARK = 1000
	...
	public void testPlayerCreation1(){
		...
		assertTrue("The player's food supply is 8: ", orderedPlayers.get(0).getFood()==8);
		assertTrue("The player's dragonFire supply is 4: ", orderedPlayers.get(0).getDragonFire()==4);
		assertTrue("The player's ore supply is 0: ", orderedPlayers.get(0).getOre()==0);
	}
	...
	public void testGetPlayerOrder1(){
		...
		controller.setDifficulty(1);
		/*
		 * Because no player has any properties, they should all be viewed equally by the 
		 * priority queue they're entered in in getPlayerOrder().  0 properties cancels out
		 * any gold advantages.
		 */
		...
		assertTrue("Third player has score = 0: ", equalPlayers.get(1).getScore()==0);
		assertTrue("Second player has score = 0: ", equalPlayers.get(2).getScore()==0);
		assertTrue("First player has score = 0: ", equalPlayers.get(3).getScore()==0);
		
		//Ensures all players were added to the PQ based on creation time (stable)
		assertTrue("First player is named Last: ", "Last".equals(equalPlayers.get(0).toString()));
		assertTrue("Second player is named Third: ", "Third".equals(equalPlayers.get(1).toString()));
		assertTrue("Third player is named Second: ", "Second".equals(equalPlayers.get(2).toString()));
		assertTrue("Fourth player is named First: ","First".equals(equalPlayers.get(3).toString()));
